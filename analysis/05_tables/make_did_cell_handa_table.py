"""
make_did_cell_handa_table.py

Purpose:
  Build the appendix cell-level difference-in-differences table for the Handa
  et al. (2025) automation and augmentation exposure measures, from
  analysis/output/coefficients/coef_microdata_did_cell_handa.csv. This is the
  Handa-measure counterpart of make_did_cell_table.py (which builds the main
  Eloundou Table referenced as \\label{tab:did_cell}).

  The private-sector fragment is the appendix table referenced as
  \\label{tab:did_cell_handa}. Structurally identical to the main table, but the
  two stacked panels are the two Handa measures (automation, augmentation)
  rather than the two outcomes; the outcome shown is employment (Poisson), the
  paper's headline margin. The estimation artifact also carries new_hires and
  log_wage, so additional panels can be added here without re-estimating.

Writes a caption-less LaTeX fragment (just the tabular; no \\begin{table},
\\caption or \\label). The float, caption and label live in paper/*.tex around
\\input{} (see CLAUDE convention: captions belong in paper/*.tex).

  private (appendix) -> analysis/output/tables/appendix_did_cell_handa.tex

Usage:
    python analysis/05_tables/make_did_cell_handa_table.py
"""

# os: build file paths and create the output directory
import os
# pandas: read the coefficient CSV and filter by measure/sector/outcome/quintile/age
import pandas as pd

# Repo root, relative to this script (analysis/05_tables/ -> ../../)
BASE_DIR = os.path.join(os.path.dirname(__file__), "..", "..")
# Cell-level Handa DiD coefficient estimates (one row per measure x sector x outcome x age x quintile)
...
# (measure key in CSV, panel label). Panels stacked in the table: one per Handa
# measure, employment outcome only.
PANELS = [
    ("automation", "Panel A. Automation exposure (Employment, Poisson)"),
    ("augmentation", "Panel B. Augmentation exposure (Employment, Poisson)"),
]
# The single outcome shown in every panel
OUTCOME = "employment"
...
def build_tabular(df_sec):
    ...
    # One panel per Handa measure (automation, augmentation)
    for pi, (measure, label) in enumerate(PANELS):
        # Italic spanning header naming the panel
        lines.append(r"\multicolumn{%d}{l}{\textit{%s}} \\" % (ncol + 1, label))
        # Rows for this measure and the employment outcome only
        sub = df_sec[(df_sec["measure"] == measure) & (df_sec["outcome"] == OUTCOME)]
    ...
    # Footer: occupations and observations. The occupation universe is the same
    # for both measures, so take it from the automation employment rows.
    emp = df_sec[(df_sec["measure"] == "automation") & (df_sec["outcome"] == OUTCOME)]
...
# Read coefficients, build the private-sector fragment, and write it out.
def main():
    # Load all cell-level Handa DiD coefficients
    df = pd.read_csv(COEF_FILE)
    # Ensure the output directory exists
    os.makedirs(OUT_DIR, exist_ok=True)

    # Private sector (2) appendix fragment
    frag = build_tabular(df[df["sector"] == 2])
    # Full output path
    path = os.path.join(OUT_DIR, "appendix_did_cell_handa.tex")
    # Write the fragment
