# =============================================================================
# ca_did_yagan.R : CA pooled post-treatment DiD with Yagan (2020) ratio
#                  outcome against a 12-month pre-window baseline.
# =============================================================================
# Mirrors ca_did_stdexp.R but replaces Poisson PPML on raw counts with linear
...
#
# Per anchor the 12-month pre-window is k in [-12, -1].
# kk encoding: each pre-period month is its own control level, all post
# months collapse to "POST" with k = -1 the reference month. The POST
# coefficient is the average post-period effect relative to the reference
# month, conditional on the within-pre-period controls.
#
# Three nested specifications per (outcome x timing x age):
#   M1 (exp):   y_ratio ~ i(kk, z_exp)                          | yrke4 + k
#   M2 (wage):  y_ratio ~ i(kk, z_wage)                         | yrke4 + k
#   M3 (full):  y_ratio ~ i(kk, z_exp) + i(kk, z_wage)
#                       + i(kk, z_expwage)                      | yrke4 + k
#
# Outputs:
#   analysis/output/coefficients/coef_ca_did_yagan.csv
#     schema: outcome, timing, age_group, model, term,
#             coef, se, n_obs, n_occ
#   analysis/output/coefficients/coef_ca_did_yagan_modelstats.csv
#     schema: outcome, timing, age_group, model,
#             nobs, n_clusters, r2
# =============================================================================
...
# -----------------------------------------------------------------------------
# balance() + kk encoding (identical to ca_did_stdexp.R)
# -----------------------------------------------------------------------------
...
# Harvest POST coefficient on `var` (handles either name order)
...
for (oc in c("count", "nyjobb")) {
    ...
    for (cfg_name in names(CONFIGS)) {
        ...
        for (a in ALDER_KEEP) {
            ...
            # Yagan ratio: cells with base = 0 dropped (ratio undefined,
            # feols rejects weight = 0).
            ...
            # Regression sample size and cluster count come from feols,
            # not the pre-filter panel size.
