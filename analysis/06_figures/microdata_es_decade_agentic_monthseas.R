# =============================================================================
# microdata_es_decade_agentic_monthseas.R : Agentic-anchored Poisson event-study
#                                           with quintile x calendar-month FE.
# =============================================================================
# Parallel to microdata_es_decade_agentic.R, but adds a quintile-by-calendar-
# month fixed effect (ai_q^cal_month) to absorb differential seasonality
# across exposure quintiles.
#
# Identifikasjons-merknad: agentic-vinduet er kort (2023m7 -> 2026m2, ~32 mnd),
# saa hver (q, kalendermaaned) har bare 2-3 observasjoner. cal_month-FE-en
# absorberer noe behandlingseffekt naar treatment-perioden inneholder samme
# kalendermaaned som referansene. Det er en kjent kostnad ved aa kontrollere
# for kvintil-spesifikk sesong.
#
# Spec (per decade age group a):
#   log E[count_{j,t}] = alpha_j + beta_t + delta_{q,m(t)}
#                     + sum_{q in {1,2,4,5}, k != -1} gamma_{q,k}
#                       * 1{ai_q(j)=q} * 1{k(t)=k}
#   k = months since April 2025 (Apr 2025 = -1). Reference: q=3.
#   m in 1..12. Window: 2023m7 through end of data. Cluster SE at occupation.
#
...
# Output: analysis/output/coefficients/coef_microdata_es_decade_agentic_monthseas.csv
# =============================================================================
...
OUT_CSV   <- file.path(BASE, "analysis", "output", "coefficients",
                       "coef_microdata_es_decade_agentic_monthseas.csv")
