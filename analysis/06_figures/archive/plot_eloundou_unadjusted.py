...
def get_age_label(alder_gr: str) -> str:
    if alder_gr in AGE_MAP:
        return AGE_MAP[alder_gr]
    if alder_gr in AGE_50PLUS:
        return '50+'
    return ''
...
# ---------------------------------------------------------------------------
def plot_figure1(counts: list[dict]) -> None:
    ...
    for ax, (occ_label, occ_codes) in zip(axes.flatten(), FIGURE1_OCCUPATIONS):
        ...
        for age_label in AGE_LABELS_ORDERED:
            raw = {dt: val for (al, dt), val in agg.items() if al == age_label}
            if not raw:
                continue
    ...
    fig.suptitle('Employment by age group\n'
                 'Headcount (October 2022 = 1)',
                 fontweight='semibold')

    note = (
        'Notes: Employment counts from Norwegian A-ordningen register data, '
    ...
    out = FIG_DIR / 'figure1_occupations_by_age_unadjusted.pdf'
...
# ---------------------------------------------------------------------------
def plot_figure2(counts: list[dict], exposure: dict[str, int]) -> None:
    norm_dt = date_to_dt(NORM_DATE)
    quintile_map = exposure  # already quintile assignments
    ...
    for i, age_label in enumerate(AGE_LABELS_ORDERED):
        ...
        for q in range(1, 6):
            raw = {dt: val for (al, qq, dt), val in agg.items()
        ...
        raw_all = {dt: val for (al, dt), val in agg_overall.items()
                   if al == age_label}
        if raw_all:
    ...
    fig.autofmt_xdate(rotation=0, ha='center')
    fig.suptitle('Employment by age and AI-exposure quintile\n'
                 'Headcount (October 2022 = 1)',
                 y=1.03, fontweight='semibold')

    # Brynjolfsson-style note at bottom
    note = (
        'Notes: Employment counts from Norwegian A-ordningen register data, '
        'normalized to October 2022 = 1. '
        'Dashed vertical line marks the\n'
        ...
        'Eloundou et al. (2024), mapped to STYRK-08\n'
        'via SOC 2010/ISCO-08 crosswalk (BLS, 2012). '
    ...
    out = FIG_DIR / 'figure2_age_by_quintile_unadjusted.pdf'
    fig.savefig(str(out), dpi=200, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved {out}")
