# =============================================================================
# microdata_es_decade_agentic_kontantlonn.R : Agentic-anchored OLS event-
#                                             study on log(kontantlonn).
# =============================================================================
# Parallel to microdata_es_decade_agentic.R but the outcome is log of mean
# monthly cash earnings, with cell count weights. Anchor: April 2025 = k=-1
# (last pre-agentic month). Window: 2023m07 onward, matching BCC's 22-month
# pre-period.
#
# Spec (per decade age group a):
#   log(kontantlonn_{j,t}) = alpha_j + beta_t
#                          + sum_{q != 3, k != -1} gamma_{q,k}
#                            * 1{ai_q(j)=q} * 1{k(t)=k}
#   k = months since April 2025. Reference: q=3.
#   Weights: cell headcount (count). Cluster SE at occupation.
#   Sample: private sector, count >= 10.
#
...
# Output: analysis/output/coefficients/coef_microdata_es_decade_agentic_kontantlonn.csv
# =============================================================================
...
WINDOW_FROM <- as.IDate("2023-07-16")     # 22 pre-period months (BCC)
...
for (a in ALDER_KEEP) {
    ...
    cat(sprintf("\n--- age group %s, n=%d, n_occ=%d ---\n",
                a, n_obs, n_occ))
    fit <- tryCatch(
        feols(log(kontantlonn) ~ i(k, ai_q, ref = -1, ref2 = "3")
              | yrke4 + k,
