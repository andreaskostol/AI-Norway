"""
plot_q3_outcomes_grid.py
========================
Generates four q3-style event-study grids -- one per outcome -- so they can
be browsed alongside the existing figure_microdata_poisson_es_grid_q3.pdf
(count).

Outputs in analysis/output/figures/:
  figure_microdata_poisson_es_grid_q3_kontantlonn.pdf
  figure_microdata_poisson_es_grid_q3_hourly_wage.pdf
  figure_microdata_poisson_es_grid_q3_nyjobb.pdf

(The count figure already exists at figure_microdata_poisson_es_grid_q3.pdf.)
"""
import os
...
# Ranges sized to the actual coef + CI envelope per outcome:
#   kontantlonn:  coef [-0.20, +0.14], CI [-0.28, +0.20] -> ylim [-0.30, +0.20]
#   hourly_wage:  coef [-0.21, +0.14], CI [-0.30, +0.20] -> ylim [-0.30, +0.20]
#   nyjobb:       coef [-0.62, +0.83], CI [-0.97, +1.50] -> ylim [-1.0,  +1.0]
OUTCOMES = [
    ("coef_microdata_es_decade_q3_kontantlonn.csv",
     "figure_microdata_poisson_es_grid_q3_kontantlonn.pdf",
     -0.30, 0.20, 0.10),
...
def make_figure(coef_path, fig_path, ylo, yhi, ystep):
    ...
    # Generate yticks at multiples of ystep within [ylo, yhi], anchored at 0.
    import numpy as np
    yticks = np.arange(round(ylo / ystep) * ystep,
                       yhi + 1e-9, ystep)
