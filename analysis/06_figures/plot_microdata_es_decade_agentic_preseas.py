"""plot_microdata_es_decade_agentic_preseas.py

Agentic-anchored Poisson event-study figure with PRE-PERIOD-estimated
quintile x cal_month seasonal adjustment applied as Poisson offset.

Reads coef_microdata_es_decade_agentic_preseas.csv.

Output: analysis/output/figures/figure_microdata_es_decade_agentic_preseas.pdf
"""
...
# Use cluster-bootstrap-corrected SE (se column = bootstrap SE; se_naive is
# the original cluster-robust step-2 SE without first-step uncertainty).
...
def main():
    ...
    ymax = min(0.35, max(0.10,
                         d["hi"].abs().max(), d["lo"].abs().max()))
    ...
    def _lab(q):
        if q == 1: return "Q1 (least exposed)"
        if q == 5: return "Q5 (most exposed)"
        return f"Q{q}"
    ...
    out = os.path.join(FIG_DIR,
                       "figure_microdata_es_decade_agentic_preseas.pdf")
