# canaries_no_wages_usage_patterns_by_age Data Dictionary

## Files Included

- `canaries_no_wages_usage_patterns_by_age.csv`: Underlying series.
- `canaries_no_wages_usage_patterns_by_age_yoy_change.csv`: Year-over-year change.
- `canaries_no_wages_usage_patterns_by_age_annualized.csv`: Annualized growth.
- `canaries_no_wages_usage_patterns_by_age_data_dictionary.md`: Documentation.
...
Facets `usage_pattern` and `age_bucket`; value columns: Wage Index (FTE-adjusted average monthly cash wage) per Claude-usage group (Handa et al. 2025 via styrk08_usage_groups.csv), following BCC (2025) Figure 3. `No usage` = canaries-sample occupations below the query threshold.
