...
To isolate the employment response specific to this newer generation of tools, we re-normalize every series to early 2025 and repeat the event-study analysis, so that $k=-1$ is the last pre-agentic month and the post-period covers the agentic window; the specification is otherwise identical to the baseline of Section~\ref{sec:regression:age}.\footnote{The re-normalization uses April 2025 as the reference month $k=-1$; results are similar for nearby reference months.} Figure~\ref{fig:agentic_es} reports the re-normalized Poisson event study by decade age group. Among the youngest workers the most-exposed quintile begins to fall below the least-exposed from the second half of 2025, the pattern foreshadowed in Section~\ref{sec:results:auto_aug}; the gap is recent and rests on few post-period months, so we assess its robustness in Section~\ref{sec:regression:robust}.

\begin{figure}[tp]
  \centering
  \includegraphics[width=\textwidth]{../analysis/output/figures/figure_microdata_es_decade_agentic_preseas.pdf}
  \caption{Poisson event-study coefficients by AI-exposure quintile and decade age group, private sector, re-normalized to the agentic window.}
  \label{fig:agentic_es}
  \begin{minipage}{\textwidth}\footnotesize\textit{Notes:} Coefficients $\gamma_{q,k}$ for $q\in\{2,\dots,5\}$ relative to $q=1$, in log points, from the cell-level Poisson event study of Section~\ref{sec:regression:age} re-normalized so that the reference month $k=-1$ is April 2025; the pre-period series is seasonally adjusted. Private sector, sample 2021m1--2026m2. Shaded bands: 95\% confidence intervals clustered at occupation.\end{minipage}
\end{figure}
