...
Artificial intelligence is advancing rapidly and could reshape the labor market. Economic theory does not sign its effect on workers in advance. In task-based models, AI can substitute for labor by automating tasks that workers used to perform. It can also complement labor by raising productivity in the tasks that stay human. The net effect on employment depends on the balance between displacement, productivity gains, and the creation or reorganization of tasks \citep{acemoglu2025}. This ambiguity matters most for young workers. AI is well suited to tasks that entry-level employees often handle, such as coding, customer support, writing, and routine analysis. If AI substitutes for these tasks, entry-level jobs may move before aggregate employment does. Young workers may then act as ``canaries in the coal mine,'' an early signal of how AI shifts labor demand, hiring, and career entry \citep{brynjolfsson2025}.

A growing body of research asks whether this shift is already visible, and the evidence is mixed. High-frequency US payroll data show weaker employment growth among young workers in highly exposed occupations, and the decline is larger where AI automates rather than augments work \citep{brynjolfsson2025}.\footnote{\citet{brynjolfsson2025} use ADP private payroll records on 3.5--5 million workers per month (January 2021--September 2025) and a Poisson event study with firm-by-quintile and firm-by-time fixed effects; workers aged 22--25 in the most AI-exposed occupational quintile decline by about 15 log points relative to the least-exposed quintile, the effect is concentrated in automation-exposed occupations under the \citet{handa2025} classification, and wages show little divergence. \citet{teeselink2025} finds, in UK Revelio Labs data, that a one-standard-deviation increase in LLM exposure lowers employment by 0.3 percent, concentrated in junior and high-wage positions, and \citet{teeselink2026} report that AI exposure reduces job postings across 39 countries, more so where employment protection is stricter. \citet{lodefalk2026} replicate the design on Swedish employer--employee data (January 2019--June 2025) with employer-by-quartile and employer-by-month fixed effects and the DAIOE generative-AI index of \citet{engberg2024}; the youngest (22--25) in the most-exposed quartile fall 5.5 percent by 2025H1 while the oldest gain 1.3 percent, and a Platsbanken job-postings analysis attributes part of the decline to monetary tightening after the April 2022 Riksbank hike. Reported magnitudes use different units and baselines and are not directly comparable.} Other work finds more muted effects, pre-existing trends, or offsetting productivity gains where AI complements workers \citep{humlum2026, kauhanen2026}.\footnote{\citet{humlum2026} compare AI-chatbot adopters to non-adopters in Danish registers plus two roughly 25,000-person surveys and find precise nulls on earnings and employment (ruling out hours effects above 2 percent at encouraged-use workplaces), though their Appendix~D.2 documents aggregate early-career declines that mirror the US; their firm-level design shows workplace adoption does not drive these declines. \citet{kauhanen2026} replicate \citet{brynjolfsson2025} on Finnish registers and attribute observed trends to demographics rather than AI, consistent with earlier Finnish nulls \citep{kauhanen2024, kauhanen2025}.} A further problem is that most studies measure exposure to early general-purpose tools, with ChatGPT as the reference point. The technology is not fixed. AI capabilities keep changing. More agentic systems arrived in 2025 and moved the frontier from single-turn assistance toward tools that plan, use software, write code, and run multi-step workflows. The labor-market effect of AI is therefore a moving target. Measuring it needs high-frequency data that separate short-run noise from persistent changes in employment, hiring, wages, and the age mix of exposed occupations.

We make progress in two ways. First, we use monthly data aggregated by occupation, age, and month. This lets us study the two events that define the recent history of AI at work: the release of ChatGPT in late 2022 and the spread of agentic tools in 2025. For each event we compare young and old workers before and after. Second, we build a monthly AI labor market index for Norway, the companion to this analysis \citep{kiindeksen2026}. It tracks employment and wages by occupation, age, and AI exposure, month by month, and lets users run their own analysis and sensitivity checks on the aggregate data.

Statistics Norway makes our data available in two forms. The first is microdata.no, a remote-analysis platform with programmatic access to the population registers and aggregated cell-level export. From the employer reporting system for 2021 to 2026 we build monthly employment series. Cells are defined by occupation, age group, sector, and AI-exposure quintile. We index each series to October 2022, the month before the release of ChatGPT on November 30, 2022. We rank occupations by the Eloundou et al.\ GPT-4 exposure measure \citep{eloundou2024}, and report the Handa et al.\ automation and augmentation shares as an alternative \citep{handa2025}. The second form is linked employer-employee data. It supports a more granular analysis and lets us check whether the conclusions from the aggregated, high-frequency data hold at the level of individual firms and workers.

Our analysis yields three broad conclusions. First, the exposure gradient is weak across the full distribution but visible in selected occupations. Pooling all workers, the most-exposed quintile does not pull away from the least-exposed after ChatGPT. The case studies are sharper. Among young software developers and customer-service agents, two highly exposed groups, employment falls. Among electricians and home health aides, two low-exposure groups, it does not. The young-worker decline in selected occupations does not survive when we rank all occupations by exposure. New hires, cash earnings, and public-sector employment show no exposure gradient either.

Second, the difference-in-differences estimates are small on average, and we read them with care. For workers aged 21 to 30 the most-exposed quintile is not distinguishable from the least-exposed. The clearest age pattern sits among older workers: the 31 to 40 group gains, and the 41 to 50 group is flat to slightly negative. We focus on the youngest workers, whose employment renews through entry rather than slow turnover, so it is the margin where changes in hiring show up first. Pre-trends are not flat, so we treat the estimates as descriptive. A sensitivity analysis bears this out. For the 21 to 30 group the result is a precise null that does not survive any relaxation of parallel trends. A closer replication of the Brynjolfsson et al.\ design for ages 22 to 25 shows the gap opening only from spring 2025, but it sits at the data edge and on rejected pre-trends.

Third, the mechanism points to automation and to the agentic period. Ranking young workers by the automation share gives a modest negative gradient. Ranking them by the augmentation share gives none. The most-exposed automation quintile pulls further from the least exposed in the second half of 2025. When we re-normalize the series to early 2025 to isolate the agentic window, the youngest most-exposed workers fall relative to the least exposed, by more than any older group. The pattern is recent and rests on few months, so we read it as emerging rather than settled.

We add to the literature in two respects. We bring the evidence up to early 2026, past the end of most existing studies, with population-level coverage. And we separate the agentic period from the earlier assistant-style window, which earlier data could not do. Norway is a useful test case. A 2026 Microsoft report ranks it third in the world for population-level AI use \citep{microsoft2026ai}, and OECD data place it first among 33 European countries for AI use at work \citep{oecd2026ai}. Enterprise adoption rose from 11 percent of firms in 2021 to 30 percent in 2025 \citep{ssb2025ikt}. If AI is moving the labor market, a high-adoption economy is where the effect should appear first.

The paper proceeds as follows. Section~\ref{sec:data} describes the data and measurement. Section~\ref{sec:results} presents the descriptive evidence. Section~\ref{sec:regression} reports the regression analysis. Section~\ref{sec:discussion} discusses the findings, and Section~\ref{sec:conclusion} concludes.
