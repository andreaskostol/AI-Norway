...
% Title
\title{\textbf{AI Exposure and Early-Career Employment:\\Evidence from Norwegian Register Data}\thanks{This research has received support from the Norwegian Research Council (grant no.\ 280350). Administrative registers made available by Statistics Norway and data from microdata.no have been essential.}}
\author{
...
\begin{abstract}
\noindent
Using monthly Norwegian population registers from 2021 through early 2026, we study whether employment has shifted away from occupations more exposed to generative AI, focusing on young workers because their employment adjusts fastest through entry and turnover. Selected high-exposure occupations, including software development and customer service, show falling employment among young workers after ChatGPT's release. Across the full exposure distribution, however, the most-exposed private-sector quintile does not separate robustly from the least-exposed quintile for workers aged 21 to 30, and we find no comparable gradient in new hires or cash earnings. For workers aged 22 to 25, an employment gap opens from spring 2025, but it appears only in the last few months of data, and the two quintiles were already drifting apart before ChatGPT, so we do not read it as a clean AI effect. We read the evidence as descriptive monitoring of an emerging labor-market adjustment, not as a causal estimate of AI's employment effect: any adjustment is recent, occupation-specific, and hard to separate from pre-existing technology-sector and macroeconomic trends.
\end{abstract}
...
Cash earnings & Total cash compensation paid by the employer during the calendar month, gross of tax; includes base salary, fixed and irregular supplements, bonuses, overtime pay, and severance. Statistics Norways remove extreme outliers in the data available at mikrodata.no. When we use the individual-level data, we winsorize at the 99.9th percentile within each occupation-by-month cell, with a pooled fallback for small cells. \\[4pt]
...
\begin{figure}[tp]
  ...
  \begin{minipage}{\textwidth}\footnotesize\textit{Notes:} Private-sector headcount, seasseasonally adjusted by removing fixed calendar-month factors, estimated over 2021--2024 as the average log-deviation from a centred $2\times12$ moving-average trend, indexed to October 2022 = 1. Occupations are ranked by the Handa et al.\ automation share. Q1 (lightest) = least exposed; Q5 (darkest) = most exposed. The raw series is Appendix Figure~\ref{fig:age_by_quintile_handa_auto}.\end{minipage}
\end{figure}
...
Two extensions point the same way. Cash earnings and the job-quality and entry margins show no exposure gradient; their figures are collected in Appendix~A.
...
The regressions below summarize the exposure gradients in the descriptive figures; because pre-trends differ across occupations and age groups, we read them as disciplined descriptive evidence rather than causal estimates. We proceed from a cross-sectional quintile analysis to age-by-quintile difference-in-differences, reconcile the cell-level and within-firm estimates, replicate the \citet{brynjolfsson2025} 22--25 design, confront the non-flat pre-trends with a sensitivity analysis, and close with the automation--augmentation and agentic-AI extensions.
...
\begin{table}[tp]
  ...
  \begin{minipage}{\textwidth}\footnotesize\textit{Notes:} Seasonally adjusted private-sector employment change from February 2025 to February 2026 for each occupation, pooled over ages 21--60, regressed on exposure-quintile dummies and weighted by February 2025 employment. Panel~A ranks occupations by the \citet{handa2025} augmentation share; Panel~B by the automation share. In each panel the first row is the employment-weighted mean change by quintile and the second is the difference relative to Q1 (the base, hence no entry), with standard errors clustered at the occupation in parentheses. The two rankings are estimated jointly on a stacked occupation-level dataset, so the triple difference (the final row: the automation Q5$-$Q1 gradient minus the augmentation Q5$-$Q1 gradient) and its standard error account for the same occupation appearing in both rankings. $^{*}$, $^{**}$, $^{***}$ denote significance at the 10, 5, and 1 percent levels.\end{minipage}
\end{table}
...
This section places the Norwegian results in cross-country context, draws out the implications for how AI-related adjustment is likely to surface in a high-adoption, centrally bargained economy, describes the companion dashboard, and states plainly what the evidence can and cannot establish. The non-AI channels that could generate the same age--exposure gradient are discussed with the sensitivity analysis in Section~\ref{sec:regression:robust}.

\subsection{Cross-Country Comparison}\label{sec:discussion:crosscountry}

Table~\ref{tab:crosscountry} situates the Norwegian patterns within the cross-country evidence, separating descriptive time series (Panel~A) from formal econometric estimates (Panel~B); we read the table rather than restate it.
...
The gap between the descriptive case studies and the formal full-distribution estimates is itself the key fact, and it recurs across the countries that report both. It need not mean AI plays no role: formal designs absorb variation that may include genuine AI effects operating outside direct firm-level adoption, such as competitive pressure from AI-adopting rivals or supply-side reallocation by workers anticipating disruption. But it does mean aggregate descriptive trends, including ours, should be read with caution, because monetary tightening and the post-COVID correction move the same occupations. \citet{humlum2026}, the only study with individual-level adoption data, finds that workplace AI-chatbot adoption does not drive the Danish early-career decline; \citet{lodefalk2026} attribute much of the Swedish decline to the Riksbank rate cycle; and at the industry level \citet{bick2026} find no clear link between AI adoption and employment in either Europe or the US. Norway, Sweden, Denmark, and Finland share similar institutions yet differ in their formal results, so institutional structure alone does not explain the heterogeneity.
...
The limited wage response is consistent with Norway's two-tier bargaining system \citep{bhuller2022} channelling any AI-related adjustment toward the quantity margin in the private sector. Downward-rigid wage floors limit short-run wage adjustment; the locally negotiated drift channel could deliver a wage response over a longer horizon. The systematic exposure-quintile evidence does not show the young-worker displacement reported for the US and Sweden, and the field has not converged on a standard ``AI exposure'' definition, so the results should be read with that measurement uncertainty in mind.


\subsection{The Companion Dashboard}\label{sec:discussion:dashboard}

The analysis is paired with a public companion index, the AI labor market index at kiindeksen.no \citep{kiindeksen2026}, and the two are complementary. Because the agentic-period pattern is recent and the identification is fragile, the dashboard is not just dissemination; it is the updating device. The paper fixes a sample window, a set of specifications, and a snapshot of conclusions; the dashboard publishes the underlying monthly series --- indexed to October 2022 by occupation, age group, sector, and AI-exposure quintile --- and refreshes them as each new month of A-ordningen data arrives, so the patterns we flag as emerging can be watched as they consolidate or fade, without waiting for a new draft. It also makes the descriptive evidence inspectable: a reader can toggle seasonal adjustment and smoothing, switch exposure measure, and drill into the individual occupations behind the quintile aggregates shown here. It does not replace the regression analysis, which supplies the identification discipline and inference the figures cannot; it keeps the descriptive half of the paper a living series as new register months arrive.


\subsection{What the Evidence Can and Cannot Establish}\label{sec:discussion:scope}

It is worth stating plainly what the aggregate data settle and what they leave open. \emph{Established:} for workers aged 21--30 there is no robust Q5-vs-Q1 employment gradient, and no comparable gradient in new hires or cash earnings, while selected high-exposure occupations do show young-worker declines. \emph{Suggestive:} a late-2025 to early-2026 divergence in the narrow 22--25 sample, and a possible automation-exposed young-worker decline, both recent and resting on rejected pre-trends. \emph{Not identified:} whether AI, the post-COVID technology correction, monetary tightening, or supply-side occupational choice drives the selected-occupation pattern --- the aggregate data cannot separate them.
...
We use monthly population registers to test whether the canary pattern generalizes beyond selected occupations, and it largely does not. Among the youngest workers, whose employment renews through entry and is least confounded by demographic change, selected high-exposure occupations show falling employment --- the case-study pattern reported for the US, Sweden, and the UK --- but the systematic exposure ranking does not: the most-exposed quintile does not separate from the least-exposed for workers aged 21 to 30, and new hires and cash earnings show no comparable gradient. The one systematic gap, in the narrow 22--25 sample, opens only from spring 2025 and widens to about 35 log points by early 2026.

We do not identify what drives the selected-occupation pattern. Pre-ChatGPT trends differ across age and occupation groups, so a difference-in-differences design that assumes parallel trends can mislead here, and the late 22--25 divergence is significant only under near-exact parallel trends. AI-related adjustment, the post-COVID correction in technology hiring, monetary tightening, and supply-side reallocation could each generate the same age--exposure gradient, and the aggregate data cannot tell them apart. \citet{humlum2026} document a similar aggregate pattern in Denmark and show that observable workplace adoption of AI chatbots accounts for at most a small fraction of it; worker- and firm-level designs in \citet{brynjolfsson2025}, \citet{lodefalk2026}, and \citet{humlum2026} answer related questions at finer units of observation.

Because any AI-related adjustment is recent and occupation-specific, the question is one of monitoring as much as of estimation, which is why we pair the paper with a live monthly index that updates as new register months arrive. The broader lesson is methodological: the Norwegian register evidence shows why early AI labor-market monitoring needs full-distribution evidence, not selected-occupation narratives alone.
...
\begin{table}[htbp]
  ...
  \begin{minipage}{\textwidth}\footnotesize\textit{Notes:} Q5$-$Q1 seasonally adjusted employment gap, in percentage points, averaged over two three-month windows: the last three months before the agentic-coding tools (February--April 2025, ending at the April 2025 reference of Section~\ref{sec:regression:agentic}) and the last three months of data (December 2025--February 2026). ``Change'' is the difference between the two windows; a negative value means the most-exposed quintile lost ground relative to the least-exposed over the agentic window. Series are private-sector headcount by age group and exposure quintile, seasonally adjusted by removing fixed calendar-month factors, estimated over 2021--2024 as the average log-deviation from a centred $2\times12$ moving-average trend, indexed to October 2022 = 1. Occupations are ranked by the Eloundou et al.\ GPT-4 score.\end{minipage}
\end{table}
...
We subject the 22--25 event study to the same honest difference-in-differences sensitivity analysis as the main 21--30 sample (Section~\ref{sec:regression:robust}), applied to the Q5-vs-Q1 firm-FE coefficients. We export the full clustered variance-covariance matrix of these coefficients from the secure server, so the analysis uses the exact specification and variance-covariance of the individual-level firm-FE event study. Even under exact parallel trends ($\bar{M}=0$) the conventional interval includes zero (Table~\ref{tab:honest_did_bcc}), and the breakdown value is zero: the late-opening 22--25 gap is too imprecisely estimated in the firm-FE design to be distinguished from zero, reinforcing our reading of the recent divergence as an emerging pattern to monitor rather than an identified effect.
