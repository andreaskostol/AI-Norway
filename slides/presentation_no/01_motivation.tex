% Motivasjon: presseoppslag, oppgaveperspektivet, litteratur, bidrag.
% Illustrasjonsslidene er gjenskapt i TikZ fra den private pptx-decken.
% (Formålssliden ligger i 00_agenda.tex, rett etter tittelsliden.)

\begin{frame}{KI og fremtidens arbeidsmarked: i overskriftene}
\centering
\begin{tikzpicture}
  \useasboundingbox (-6.5, -2.7) rectangle (6.5, 2.7);
  \node<1->[rotate=-4] at (-4, 0) {%
    \includegraphics[height=5cm,keepaspectratio]{Economist_jobs_apocalypse.jpg}};
  \node<2->[rotate=3]  at (0, -0.3) {%
    \includegraphics[height=5cm,keepaspectratio]{{Atlantic_AI Has Ruined the Job Market}.JPG}};
  \node<3->[rotate=-2] at (4, 0.2) {%
    \includegraphics[height=5cm,keepaspectratio]{{Axios 2025 white collar bloodbath Amodei}.jpg}};
\end{tikzpicture}

\vspace{0.4em}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Casestudier: kanarifugl-mønsteret er reelt}
\centering
\includegraphics[height=5.6cm,keepaspectratio]{figure_occupations_by_age_sa.pdf}

\vspace{0.2em}
\footnotesize
Programvareutviklere 21--30 år faller markert etter ChatGPT (rundt $-18\,\%$ per innbygger); kundeservicemedarbeidere viser samme form, svakere. Elektrikere og hjemmehjelpere, de laveksponerte referanseyrkene, gjør det ikke.
\end{frame}

% ---------------------------------------------------------------
% To ekstra yrkescase fra dashbordet (inngår ikke i paperets casestudier).
\begin{frame}{Informasjonsrådgivere}
\centering
\includegraphics[height=5.8cm,keepaspectratio]{figure_case_informasjonsradgivere_sa.pdf}

\vspace{0.15em}
\raggedright\scriptsize
Dashbordcase, inngår ikke i paperets casestudier. STYRK-08 2432; rundt 2\,700 ansatte i privat sektor høsten 2022, den offentlige delen av yrket inngår ikke. Høy KI-eksponering (øverste femtedel). Sesongjustert og per innbygger, 100 = oktober 2022.
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Designyrker}
\centering
\includegraphics[height=5.8cm,keepaspectratio]{figure_case_designyrker_sa.pdf}

\vspace{0.15em}
\raggedright\scriptsize
Dashbordcase. STYRK-08 2163 Produkt- og klesdesignere og 2166 Grafiske- og multimediadesignere, sett under ett; rundt 5\,600 ansatte høsten 2022. KI-eksponering i nest øverste femtedel. Sesongjustert og per innbygger, 100 = oktober 2022.
\end{frame}

% ---------------------------------------------------------------
\section{En mer systematisk, helhetlig tilnærming}

% ---------------------------------------------------------------
% Illustrasjon (a): oppgaveperspektivet
\begin{frame}{En jobb består av oppgaver som påvirkes ulikt}
\textcolor{illInk}{\textbf{Teknologi kan endre deler av jobben uten å erstatte hele.}}

\vspace{0.3em}
\centering
\begin{tikzpicture}
  \useasboundingbox (0,0.2) rectangle (\textwidth,-3.85);
  \illcard{(0.0,0)}{illBlue}{OPPGAVENIVÅ}{En jobb er en samling oppgaver}{Arbeidsoppgaver kan flyttes mellom mennesker og teknologi. Yrket består så lenge viktige oppgaver fortsatt må løses av mennesker.}
  \illcard{(5.1,0)}{illOrange}{HØY EKSPONERING}{Digital tekst, kode og analyse}{Standardiserte førsteutkast, dokumentbehandling og koding ligger nær språkmodellenes styrker.}
  \illcard{(10.2,0)}{illGray}{LAVERE DIREKTE EKSPONERING}{Tilstedeværelse, relasjon og ansvar}{Fysisk arbeid, omsorg og skjønn under usikkerhet er vanskeligere å overføre fullt ut til en språkmodell. Per nå.}
\end{tikzpicture}

\vspace{0.15em}
\illbanner{Rutinepregede inngangsoppgaver kan endres før hele yrker gjør det.}

\vspace{0.1em}
\raggedright\tiny\textcolor{illGray}{Kilder: Autor, Levy \& Murnane (2003); Acemoglu \& Autor (2011); Eloundou mfl.\ (2024); Brynjolfsson, Chandar \& Chen (2025).}
\end{frame}

% ---------------------------------------------------------------
% Illustrasjon (b): Acemoglu-Restrepo-rammeverket
\begin{frame}{Tre krefter trekker sysselsettingen i ulike retninger}
\vspace{0.3em}
\centering
\begin{tikzpicture}
  \useasboundingbox (0,0.2) rectangle (\textwidth,-3.85);
  \illcard{(0.0,0)}{illRed}{FORTRENGNING}{Oppgaver flyttes til maskiner}{Arbeidskraftens oppgaveandel faller. Etterspørselen etter arbeid går ned, alt annet likt.}
  \illcard{(5.1,0)}{illTeal}{PRODUKTIVITET}{Lavere kostnad øker produksjonen}{Pris, kvalitet og etterspørsel kan endres. Behovet for arbeid i oppgavene som står igjen kan øke.}
  \illcard{(10.2,0)}{illPurple}{NYE OPPGAVER}{Arbeidskraft får nye funksjoner}{Nye produkter, kontrollbehov og arbeidsoppgaver kan gjeninnsette arbeid i produksjonen.}
\end{tikzpicture}

\vspace{0.5em}
\illbanner{Nettoeffekten følger ikke av hva modellen teknisk sett kan gjøre.}

\vspace{0.2em}
\raggedright\tiny\textcolor{illGray}{Rammeverk: Acemoglu og Restrepo (2019), Automation and New Tasks.}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Nyere litteratur: KI-eksponering og sysselsetting}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l l p{6.2cm}}
\toprule
Land & Studie & Hovedfunn (yngste arbeidstakere, øverste kvintil) \\
\midrule
USA            & Brynjolfsson, Chandar \& Chen (2025) & Q5, 22--25 år: nedgang på om lag 15 log-punkter \\
Sverige        & Lodefalk mfl.\ (2026)                & Q5, 22--25 år: nedgang på ca.\ 5,5\,\%; monoton gradient \\
Storbritannia  & Teeselink (2025)                     & $-0{,}3\,\%$ per standardavvik LLM-eksponering; konsentrert i junior- og høytlønnsstillinger \\
39 land        & Teeselink \& Carey (2026)            & Færre nyansettelser; sterkere utslag der stillingsvernet er strengt \\
Danmark        & Humlum \& Vestergaard (2026)         & Null: nedgangen for unge skyldes ikke KI-bruk i virksomhetene; utelukker effekter over 2\,\% \\
Finland        & Kauhanen \& Rouvinen (2026)          & Null; utviklingen følger demografi, ikke KI \\
\bottomrule
\end{tabular}

\vspace{0.8em}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Bidrag}
\begin{itemize}\setlength{\itemsep}{8pt}
  \item Et offentlig, månedlig dashbord, \textbf{kiindeksen.no}: sysselsettingen i hele populasjonen av private lønnstakerforhold i Norge, fulgt etter KI-eksponering med rundt tre måneders forsinkelse.
  \item Tester om «kanarifugl»-mønsteret (Brynjolfsson mfl.) generaliserer: utvalgte høyeksponerte yrker mot \textbf{hele eksponeringsfordelingen}.
  \item Undersøker den ferske effekten av \textbf{agentisk KI} (\emph{«autonome LLM-baserte systemer som planlegger, bruker verktøy og utfører flertrinnsoppgaver»}, Korinek 2025).
\end{itemize}
\end{frame}

% ---------------------------------------------------------------
% Illustrasjon (c): fire størrelser som ofte blandes sammen
\begin{frame}{Fire steg i virkningskjeden}
\vspace{0.4em}
\centering
\begin{tikzpicture}
  \useasboundingbox (-0.1,0.4) rectangle (13.8,-3.5);
  \illstep{(0.0,0)}{01}{Kapasitet}{Hva kan modellen gjøre i benchmarks?}
  \illarrow{(3.05,-1.4)}
  \illstep{(3.6,0)}{02}{Bruk}{Tas verktøyet faktisk i bruk på jobb?}
  \illarrow{(6.65,-1.4)}
  \illstep{(7.2,0)}{03}{Oppgaveendring}{Blir arbeidet raskere, bedre eller annerledes?}
  \illarrow{(10.25,-1.4)}
  \illstepx{(10.8,0)}{04}{Utfall}{Endres ansettelser, lønn eller sysselsetting?}
\end{tikzpicture}

\vspace{0.8em}
\centering
\textcolor{illInk}{\textbf{KI-indeksen observerer det siste leddet: arbeidsmarkedsutfall i norske registerdata.}}

\vspace{0.4em}
\footnotesize\textcolor{illGray}{Selskapenes egne bruksdata viser at hvert ledd filtrerer: KI brukes i to av tre yrker, men i medianyrket bare for en femdel av oppgavene.}

\vspace{0.4em}
\raggedright\tiny\textcolor{illGray}{Kilder: Narayanan \& Kapoor (2025); Eloundou mfl.\ (2024); Google (2026); kiindeksen.no.}
\end{frame}
